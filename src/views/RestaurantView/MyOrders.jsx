    ...
    return (
        <>
            <Loader loaded={!loading}>
                <MyOrderList orders={orders} onCancel={handleCancel} onQRcode={onQRcodeClick} />
            </Loader>
            <SiteModal
                open={modalOpen}
                title="ההזמנות שלי"
...
const MyOrderList = ({ orders, onCancel, onQRcode }) => {
    ...
    return (
        <ul className="order-list">
            {orders.map((order, idx) => (
                <MyOrderItem key={idx} {...order} onCancel={onCancel} onQRcode={onQRcode} />
            ))}
        </ul>
    );
}
